...
public:
	explicit PlayerMaster(mp_uint32 numDevices = DefaultMaxDevices);
	~PlayerMaster();
	
	static const char* getPreferredAudioDriverID();
	static mp_uint32 getPreferredSampleRate();
	static mp_uint32 getPreferredBufferSize();
	static mp_uint32 roundToNearestPowerOfTwo(mp_uint32 v);
	static float convertBufferSizeToMillis(mp_uint32 sampleRate, mp_uint32 bufferSize);

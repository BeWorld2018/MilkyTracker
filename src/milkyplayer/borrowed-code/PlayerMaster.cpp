...
mp_uint32 PlayerMaster::getPreferredSampleRate()
{
	AudioDriverManager audioDriverManager;
	return audioDriverManager.getPreferredAudioDriverSampleRate();
}

mp_uint32 PlayerMaster::getPreferredBufferSize()
{
	AudioDriverManager audioDriverManager;
	return audioDriverManager.getPreferredAudioDriverBufferSize();
}

mp_uint32 PlayerMaster::roundToNearestPowerOfTwo(mp_uint32 v)
{
	for (mp_uint32 i = 0; i < 32; i++)
	{
		mp_uint32 shifted = 1u << i;
		if (shifted >= v)
			return shifted;
	}
...
float PlayerMaster::convertBufferSizeToMillis(mp_uint32 sampleRate, mp_uint32 bufferSize)
{
	return ((float)(bufferSize) / (float)sampleRate) * 1000.0f;
	//return (1.0f / (44100.0f / (MP_BEATLENGTH*bufferSize)))*1000.0f;
}

PlayerMaster::PlayerMaster(mp_uint32 numDevices/* = DefaultMaxDevices*/) :
	listener(NULL),
	...
	multiChannelRecord(true)	
{
	...
	for (mp_uint32 i = 0; i < sizeof(panning) / sizeof(pp_uint8); i++)
	{
		switch (i & 3)
		{
			case 0:
